...
- [ ] Default bridge mode was **not** separately tested — given host mode already didn't reach the real LAN, and default bridge is architecturally even more isolated (a NAT'd virtual subnet inside the same VM, one more hop removed), the "reports zero real devices" prediction is expected to hold, but this specific run was not performed
- Verify: **Partially run — the single most important claim (host mode delivers real-LAN parity on macOS) turned out to be false, not merely unverified.** The plan and README have been corrected to reflect this rather than left asserting something now known to be wrong. Bridge-mode's "zero devices" claim remains a reasoned prediction, not yet independently confirmed the same way.
...
| **Total implementation time** | ~2.5–3 hours as originally scoped; the post-hoc Docker/Colima install + real verification pass (not part of the original estimate, done in a follow-up session once Docker became available) added roughly another hour, most of it Colima's own first-time VM provisioning |
| **Risk** | **Materialized, not just theoretical.** The networking-mode trade-off (Central Design Decision) was flagged as the one real risk concentration before implementation — real verification confirmed it was worse than the original two-column table suggested: `host` mode doesn't just fail to help on macOS, it makes the container unreachable from the Mac entirely, in addition to not seeing the real LAN. Caught specifically *because* Stage 4 insisted on real-network verification rather than taking the original reasoning on faith — the same discipline `dc1138f` itself was found through. Secondary risk (apt package versions producing `arp`/`netstat` output drift) did **not** materialize: `parseArpTable`/`readArpTable` worked correctly inside the container (correctly parsed the VM's own `192.168.5.0/24` neighbors, `unresolvedMacCount` behaved consistently) — the parsers themselves are fine, only the network they had access to was wrong. No CI publishing pipeline exists in this repo (unlike the sibling Takomachi project, which has `.github/workflows/ci.yml`) — publishing a prebuilt image remains out of scope for that reason. **Still open:** no genuine Linux host was available to verify the one case the whole `host`-mode recommendation actually rests on — the reasoning (no VM layer between the container and the physical LAN on real Linux) is architecturally sound, but unlike the macOS case above, it has not been empirically tested. |
