...
| **`macvlan` network** | Works — the container gets its own MAC/IP directly on the physical LAN segment — **confirmed by measurement, see below** | Linux only, and needs the host's physical interface name at compose-time; not supported by Docker Desktop's VM-based networking either | More setup complexity for a homelab operator than this app's "no build step, no bundler" philosophy (Objective 5) seems to warrant for a first cut. **Also confirmed:** the Docker host itself cannot reach the container over the macvlan's own parent interface — a known Linux kernel restriction, not a bug — while every other device on the LAN (including the Docker host via any *other* NIC it has on the same subnet) can |
...
**Empirical verification of default bridge mode, on the same genuine Linux host (2026-08-12): the "silently return zero real devices" prediction above was wrong about the mechanism, though right about the outcome.** Built and ran the same image with no `--network` flag (Docker's default bridge, the same networking `docker-compose.yml`'s commented-out `ports:` block is meant for), mapping `-p 3001:3000`, on the same Linux Mint host.

- `docker exec snd-home-verify ip addr show eth0` showed `172.17.0.2/16` — Docker's default bridge subnet, a full `/16` (65,534 usable hosts), not the `/24` this codebase's `lan/lanScanner.js` was written assuming a typical home LAN would be.
- That's large enough to trip `lan/lanScanner.js`'s own `MAX_HOSTS` safety cap (1024, see its header comment — added as a safety valve against exactly this kind of runaway enumeration, not written with Docker's bridge subnet in mind at the time). **The scan does not run at all and errors loudly**, rather than running and silently finding nothing: `GET /api/lan/status` returned `knownDeviceCount: 0, onlineCount: 0, lastError: "Refusing to enumerate 65534 hosts for 172.17.0.0/16 (exceeds the 1024-host safety cap)"`.
- System monitoring and reachability held up exactly as expected: `curl http://localhost:3001/api/health` and `/api/system` both succeeded from the host via the mapped port, returning real (if container-scoped) CPU/memory/disk figures.

**Revised understanding:** the original framing ("silently return zero real devices") undersold this — the actual failure mode is a loud, diagnosable `lastError`, not a silent empty result, which is arguably the better outcome given this codebase's own stated aversion to silently-wrong results (§27 above). `LAN_SCAN_CIDR` (already documented as the fix for multi-interface hosts) also fixes this case if an operator insists on running LAN Device Monitoring under bridge mode anyway — setting it to the container's own tiny bridge subnet would at least let the scan run, though it would still only ever see other containers, never the real LAN, so this isn't a real workaround, just an explanation of why the error is a safety cap and not a hard limitation of the networking mode itself.

**Empirical verification of `macvlan`, same genuine Linux host (2026-08-12): works as designed, plus one caveat the table's original wording didn't anticipate.** `docker network create -d macvlan --subnet=192.168.1.0/24 --gateway=192.168.1.1 -o parent=enxd83062a59025 snd_macvlan` (the host's wired NIC — macvlan needs a real physical parent, and this host also has a WiFi NIC on the same subnet, `wlp1s0b1`, used for the host-reachability check below), then `docker run --network snd_macvlan --ip 192.168.1.199 ...`.

- **The container got a real, independent identity on the physical LAN** — `docker exec ... ip addr show eth0` showed `192.168.1.199/24` with its own MAC, directly on the same segment as every other device, no NAT and no virtual bridge in between.
- **Reachable from elsewhere on the real LAN, with real LAN data.** `curl http://192.168.1.199:3000/api/lan/status` from the Mac (a separate physical machine on the same LAN) succeeded immediately and returned `knownDeviceCount: 13, onlineCount: 15, unresolvedMacCount: 1` — the real device population, not an isolated subnet's. `docker exec snd-home-verify arp -an` showed the same real neighbors (`192.168.1.1`, `.21`, `.48`, `.90`, `.106`, `.135`, `.144`, `.192`, `.193`, `.237`, `.242`, `.250` — the same set seen in the `network_mode: host` run above) plus the Docker host's own two LAN-facing addresses.
- **The Docker host itself could not reach the container over the macvlan's own parent interface** — `curl --interface enxd83062a59025 http://192.168.1.199:3000/api/health` timed out (and a plain, unbound `ping`/`curl` from the host also failed, since its default route happens to prefer that same interface). This is a well-known Linux kernel restriction, not a bug in this app or a misconfiguration: a macvlan parent interface and its own macvlan children can't exchange traffic directly. Confirmed as specifically a parent-interface issue, not a host-vs-container issue in general, by retrying `curl --interface wlp1s0b1 http://192.168.1.199:3000/api/health` (the host's *other* NIC, not the macvlan's parent) — **that succeeded immediately.** A single-NIC homelab host running this in `macvlan` mode would need the standard workaround (a macvlan shim/sub-interface on the host itself) to reach its own container's dashboard directly; this dual-NIC test host happened to have a second physical path that made the limitation visible without needing that workaround to demonstrate it.

**Conclusion:** `macvlan`'s row in the table above is now measured, not just reasoned from how the option is generally understood to work: real LAN identity and data, confirmed, at the cost of the host-reachability caveat above — worth calling out explicitly in README given this app's own dashboard is meant to be reached by the operator, not just other devices.
...
- Verify: **RUN, 2026-08-13, via Colima, with real host-local data actually present at the time of the test** (this machine's own `data/alertRules.json`, `data/lanDevices.json`, and `data/lan-status-check.log` — genuine, non-empty, non-synthetic files, not test fixtures) — closes the gap this line used to describe as "still not explicitly checked." `docker build -t snd-home-dockerignore-check .` reported `Sending build context to Docker daemon  869.9kB`, against a raw (un-filtered) working-tree size of `12M` (`du -sh .`) — a real, measured, order-of-magnitude drop, not merely "expected to drop." Directly inspected the built image's filesystem: `docker run --rm snd-home-dockerignore-check ls -la /app/data` showed only `.gitkeep` — none of the three real host data files were baked in. Also confirmed `.env` and `.git` are absent from the image (`.env.example` is present, correctly — it isn't excluded, and isn't meant to be), and `node_modules` is present but populated by the image's own `npm ci` layer, not copied from the host (expected, unrelated to this check). Test image removed after inspection (`docker rmi`), no artifacts left behind.
...
### Stage 4 — LAN Device Monitoring parity check (all networking modes) *(30 min)*
...
- [x] **Default bridge mode, run 2026-08-12 (same Linux Mint host): scan refuses to run, rather than running and finding zero.** `docker run` with no `--network` flag, `-p 3001:3000`. `docker exec ... ip addr show eth0` showed `172.17.0.2/16` — Docker's default bridge, a `/16` (65,534 hosts) — which trips `lan/lanScanner.js`'s own `MAX_HOSTS=1024` safety cap. `GET /api/lan/status` returned `knownDeviceCount: 0, onlineCount: 0, lastError: "Refusing to enumerate 65534 hosts for 172.17.0.0/16 (exceeds the 1024-host safety cap)"` — a loud, diagnosable error, not a silent empty scan. System monitoring and reachability were unaffected: `curl http://localhost:3001/api/health` and `/api/system` both succeeded from the host via the mapped port. Not separately tested on macOS/Colima, but Colima's own bridge subnet is expected to be the same `172.17.0.0/16` default, so the same safety-cap error is expected there too.
- [x] **`macvlan`, run 2026-08-12 (same Linux Mint host): works as designed, with a host-reachability caveat.** `docker network create -d macvlan --subnet=192.168.1.0/24 --gateway=192.168.1.1 -o parent=enxd83062a59025 snd_macvlan`, `docker run --network snd_macvlan --ip 192.168.1.199 ...`. From another machine on the real LAN, `curl http://192.168.1.199:3000/api/lan/status` returned `knownDeviceCount: 13, onlineCount: 15, unresolvedMacCount: 1` — real LAN data, not an isolated subnet. `docker exec snd-home-verify arp -an` showed the same real neighbors as the `network_mode: host` run. However, **the Docker host itself could not reach the container over the macvlan's parent interface** (`curl --interface enxd83062a59025 ...` timed out) — a known Linux kernel restriction on macvlan parent/child traffic, not a bug; confirmed as interface-specific (not host-vs-container in general) since `curl --interface wlp1s0b1 ...` (the host's other, non-parent NIC) succeeded immediately. See the Central Design Decision section's third "Empirical verification" entry for the full account.
- Verify: **All three networking modes this plan's table describes are now empirically verified on genuine Linux, plus host mode's macOS/Colima failure mode.** `network_mode: host`: confirmed full parity on Linux, falsified on macOS/Colima. Default bridge: confirmed non-functional for LAN Device Monitoring on Linux (safety-cap error, not the originally-predicted silent zero-devices result), system monitoring/alerting/reachability unaffected. `macvlan`: confirmed full real-LAN parity and data on Linux, with the newly-documented host-reachability caveat. No remaining open verification items for this stage.
