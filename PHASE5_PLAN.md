...
- [ ] 1.1 Define the rule schema + a pure `validateRule(rule)` function in `alerts/ruleStore.js` *(30 min)*
- [ ] 1.2 Implement in-memory CRUD: `create`, `list`, `get`, `update`, `remove` *(30 min)*
- [ ] 1.3 Add JSON-file persistence: load `data/alertRules.json` on startup, write-through on every mutation *(30 min)*
- [ ] 1.4 Unit tests: CRUD behavior + persistence round-trip (write, restart simulation, reload) *(30 min)*

### Stage 2 — Rule evaluator (pure state-machine logic)
- [ ] 2.1 Define state constants and the runtime-state shape `{ state, breachSince, clearSince, lastNotifiedAt, alertId }` *(15 min)*
- [ ] 2.2 Implement dot-path metric extraction with null-safety (`resolveMetric(snapshot, "disk.percent")`) *(20 min)*
- [ ] 2.3 Implement the operator comparison function (`>`, `>=`, `<`, `<=`, `==`, `!=`) *(15 min)*
