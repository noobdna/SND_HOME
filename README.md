...
- [ ] **Phase 6 — Distributed & Self-Hosted Platform** 📋 *Planned*
  - [ ] Docker support — `Dockerfile`/`docker-compose.yml`/`.dockerignore` per [`PHASE6_DOCKER_PLAN.md`](PHASE6_DOCKER_PLAN.md); `npm test` unaffected (356/356). Docker was later installed (via Colima) and the image was actually built and run against a real Mac/LAN: `docker build` and container startup/healthcheck work correctly, but the LAN-parity claim this item originally rested on turned out to be **wrong for macOS** — `network_mode: host` there is scoped to the VM Colima/Docker Desktop run containers in, not the real Mac, so it neither reaches the real LAN nor is reachable from the Mac's own `localhost` (see the [Docker section](#-docker) above for the measured numbers). README and the plan doc have been corrected accordingly. **Still not checked off**: the one case the whole `host`-mode design decision depends on — a genuine Linux host, with no VM layer in between — has not actually been tested, only reasoned about
...
| **Any platform, default bridge** (`ports:` mapping, in the compose file's comments) | ✅ Full | ❌ Reports zero devices — an isolated NAT'd subnet, one layer further removed than the VM case above; not separately re-verified after the host-mode finding above, but expected to hold | ✅ Yes (that's what the `ports:` mapping is for) |

**In practice, on macOS:** use default bridge mode (comment out `network_mode: host` in `docker-compose.yml`, uncomment the `ports:` block) if you want to reach the dashboard and use system monitoring + alerting — LAN Device Monitoring will report zero devices either way on a Mac, so `host` mode buys you nothing there except losing `localhost` access to your own dashboard. `network_mode: host` is only worth using on an actual Linux machine. On a host with more than one active network interface, `LAN_SCAN_CIDR` (see the environment variable table above) is the escape hatch if auto-detection picks the wrong one inside the container — relevant on Linux, moot on macOS until the VM-networking gap above is addressed some other way.
